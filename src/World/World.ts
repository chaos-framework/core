...
export abstract class World implements ComponentContainer, Listener {
  ...
  addEntity(entity: Entity, changes?: NestedSetChanges): NestedSetChanges | boolean {
    if (!this.entities.has(entity.id) && this.isInBounds(entity.position)) {
      ...
      if (entity.active) {
        ...
        entity.visibleChunks.addSet(
          new Set<string>(chunksInView.map((v) => this.getFullChunkID(v.x, v.y))),
          undefined,
          changes
        );
  ...
  getChunksInView(center: Vector, distance: number): Vector[] {
    // TODO SCOPE unit test
    const chunks: Vector[] = [];
...
export namespace World {
  ...
  export function deserializeAsClient(json: World.SerializedForClient): World {
    return new ClientWorld({
      ...json,
      size: new Vector(json.width, json.height),
      baseLayer: new ByteLayer(0)
    });
  }
}
