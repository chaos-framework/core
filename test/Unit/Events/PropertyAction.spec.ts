...
import { Entity, PropertyChangeAction } from '../../../src/internal.js';

describe('PropertyChange Action', () => {
  let e: Entity;
  beforeEach(() => {
    e = new Entity({ name: "Test Entity" });
    e._addProperty('HP', 50, 0, 100)
  });

  it('Calculates adjustments (add/subtract) correctly.', () => {
    const a = new PropertyChangeAction({caster: e, target: e, property: e.getProperty('HP')!, amount: 100});
    a.adjust(10);   // 100 + 10
    a.adjust(-10);  // 100 + 0
    a.adjust(30);   // 100 + 30
