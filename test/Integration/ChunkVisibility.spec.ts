  ...
  describe('Entities and worlds tracking visible chunks', function () {
    describe('Active entities', function () {
      ...
      it('Tracks and forgets chunks when moving through a world', async function () {
        ...
        expect(action.chunkVisibilityChanges.removed['world'][streamingWorld.id]).to.contain(
          streamingWorld.getFullChunkID(0, 0)
        );
        expect(action.chunkVisibilityChanges.added['entity'][entityA1.id]).to.contain(
          streamingWorld.getFullChunkID(2, 2)
        );
        expect(action.chunkVisibilityChanges.removed['entity'][entityA1.id]).to.contain(
          streamingWorld.getFullChunkID(0, 0)
        );
      });
      ...
      it('Updates chunks appropriately after a ChangeWorldAction', async function () {
        ...
        expect(changeWorlds.chunkVisibilityChanges.removed['entity'][entityA1.id]).to.include(
          fixedWorld1.getFullChunkID(0, 0)
        );
        expect(changeWorlds.chunkVisibilityChanges.added['world'][streamingWorld.id]).to.include(
          streamingWorld.getFullChunkID(0, 0)
        );
        expect(changeWorlds.chunkVisibilityChanges.added['entity'][entityA1.id]).to.include(
          streamingWorld.getFullChunkID(0, 0)
        );
      });
